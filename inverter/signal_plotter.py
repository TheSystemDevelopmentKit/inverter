...
class signal_plotter(thesdk):
    ...
    def main(self):
        ...
        if (
            self.plotmodel == "eldo"
            or self.plotmodel == "spectre"
            or self.plotmodel == "ngspice"
        ):
            ...
            axes[1, 0].grid(True)
            axes[0, 1].plot(
                self.IOS.Members["A_OUT"].Data[:, 0],
                self.IOS.Members["A_OUT"].Data[:, 1],
                label="Input",
            )
            axes[0, 1].grid(True)
            axes[1, 1].plot(
                self.IOS.Members["Z_ANA"].Data[:, 0],
                self.IOS.Members["Z_ANA"].Data[:, 1],
                label="Output",
            )
            axes[1, 1].plot(
                self.IOS.Members["Z_RISE"].Data[:, 0],
                np.ones(self.IOS.Members["Z_RISE"].Data[:, 0].shape)
                * self.plotvdd
                / 2,
                ls="None",
                marker="o",
            ...
            axes[0].grid(True)
            axes[1].stem(
                x, self.IOS.Members["Z"].Data[latency : nsamp + latency, 0]
            )
    ...
    def run(self, *arg):
        if self.model == "py":
            ...
            self.print_log(
                type="E", msg="Model %s not supported" % (self.model)
            )
