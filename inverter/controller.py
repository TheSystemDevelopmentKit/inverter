...
class controller(rtl):
    ...
    def __init__(self, *arg, **kwargs):
        ...
        self.step = int(
            1 / (self.Rs * 1e-12)
        )  # Time increment for control in simulation units. Assumed to be ps
        self.time = 0
        self.IOS = Bundle()
        ...
        if self.lang == "sv":
            self.dut = verilog_module(file=self.vlogsrcpath + "/inverter.sv")
        elif self.lang == "vhdl":
            self.dut = vhdl_entity(file=self.vhdlsrcpath + "/inverter.vhd")
